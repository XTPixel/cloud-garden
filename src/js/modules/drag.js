(() => {
  const { storageKeys, readStorage, writeStorage, clamp } = Garden;
  ...
  function applyDefaultLayout() {
    ...
    defaultLayout.forEach(([name, baseX, baseY]) => {
      ...
      applyPosition(widget, clamp(x, margin, maxX), clamp(y, margin, maxY));
    });
  }
  ...
  Garden.applySavedLayout = function applySavedLayout() {
    const layout = getLayout();
  ...
  Garden.resetLayout = function resetLayout() {
    localStorage.removeItem(storageKeys.layout);
    applyDefaultLayout();
  };

  Garden.enableDragging = function enableDragging({ onPositionSaved, onNotesClick } = {}) {
    document.querySelectorAll('[data-widget]').forEach((widget) => {
      let startX = 0;
      let startY = 0;
      let originX = 0;
      ...
      widget.addEventListener('pointerup', (event) => {
        ...
        if (widget.classList.contains('notes-fab') && !didMove) onNotesClick?.();
      });
