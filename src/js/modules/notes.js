...
import { createId, escapeHtml, clamp } from '../utils/format.js';
...
export function addNote(onSaved) {
  const notes = getNotes();
...
export function updateNoteText(event) {
...
export function startNoteDrag(event) {
  const noteElement = event.target.closest('.sticky-note');
...
export function enableNoteDragging(options = {}) {
